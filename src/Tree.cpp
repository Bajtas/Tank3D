...
GLuint Tree::texture;
void Tree::loadModel(const char *Path) {
...
Tree::Tree(string Path) : r(1), g(1), b(1) {
	TextureHandler::setTexture(texture, Path);
}


void Tree::setTexture(std::string Path) {
	TextureHandler::setTexture(texture, Path);
}
void Tree::randPosition(float x, float z){
	this->x = x;
	this->z = z;
}
void Tree::draw() {
