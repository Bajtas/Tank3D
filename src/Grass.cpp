...
void Grass::loadModel(const char *Path) {
...
Grass::Grass(string Path) : r(1), g(1), b(1) {
	TextureHandler::setTexture(texture, Path);
}


void Grass::setTexture(std::string Path) {
	TextureHandler::setTexture(texture, Path);
}
void Grass::randPosition(float x, float z){
	this->x = x;
	this->z = z;
}
void Grass::draw() {
